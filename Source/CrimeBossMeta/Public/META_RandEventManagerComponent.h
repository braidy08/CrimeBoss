...
protected:
    ...
    UPROPERTY(BlueprintReadWrite, EditAnywhere, meta=(AllowPrivateAccess=true))
    TArray<UMETA_Character*> RandomEventRewardHeisters;
...
public:
    ...
    UFUNCTION(BlueprintCallable, BlueprintPure)
    FGameplayTag GetEventIdForMissingHeister(const FMETA_CharacterID& inCharacterID) const;
    
    UFUNCTION(BlueprintCallable, BlueprintPure)
    void GetEventDataForMissingHeister(const FMETA_CharacterID& inCharacterID, FMETA_MissingHeisterEventData& outData, bool& outSuccess) const;
    
    UFUNCTION(BlueprintCallable)
    UMETA_Character* GenerateRandomHeisterForRandomEventReward(const FGameplayTag inEventTag, const FMETA_RandEventReward& inRewardData, UPARAM(Ref) FMETA_RandEventRewardRuntimeData& inOutComputedReward);
