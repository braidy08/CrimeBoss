...
FGameplayTag UMETA_RandEventManagerComponent::GetEventIdForMissingHeister(const FMETA_CharacterID& inCharacterID) const {
    return FGameplayTag{};
}

void UMETA_RandEventManagerComponent::GetEventDataForMissingHeister(const FMETA_CharacterID& inCharacterID, FMETA_MissingHeisterEventData& outData, bool& outSuccess) const {
}

UMETA_Character* UMETA_RandEventManagerComponent::GenerateRandomHeisterForRandomEventReward(const FGameplayTag inEventTag, const FMETA_RandEventReward& inRewardData, FMETA_RandEventRewardRuntimeData& inOutComputedReward) {
    return NULL;
}
