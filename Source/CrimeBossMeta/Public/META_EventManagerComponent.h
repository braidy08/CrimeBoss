...
protected:
    ...
    UPROPERTY(BlueprintReadWrite, EditAnywhere, meta=(AllowPrivateAccess=true))
    TArray<TSoftObjectPtr<UStoryGraphManager>> StartedPlotlines;
...
protected:
    UFUNCTION(BlueprintCallable)
    void FilterByPreviouslyStartedPlotlines(const TArray<TSoftObjectPtr<UStoryGraphManager>>& inPlotlineGraphs, const int32 inUsedPlotlinesCount, TArray<TSoftObjectPtr<UStoryGraphManager>>& outPlotlineGraphs);
...
public:
    ...
    UFUNCTION(BlueprintCallable, BlueprintNativeEvent)
    bool AddHeisterToEventByID(FGameplayTag inEventTag, const FMETA_CharacterID inCharacterID, bool inUseInternalCheckForState);
    
    UFUNCTION(BlueprintCallable, BlueprintNativeEvent)
    bool AddHeisterToEventByCharacter(FGameplayTag inEventTag, const UMETA_Character* inCharacter);
