...
void UMETA_EventManagerComponent::FilterByPreviouslyStartedPlotlines(const TArray<TSoftObjectPtr<UStoryGraphManager>>& inPlotlineGraphs, const int32 inUsedPlotlinesCount, TArray<TSoftObjectPtr<UStoryGraphManager>>& outPlotlineGraphs) {
}
...
bool UMETA_EventManagerComponent::AddHeisterToEventByID_Implementation(FGameplayTag inEventTag, const FMETA_CharacterID inCharacterID, bool inUseInternalCheckForState) {
    return false;
}

bool UMETA_EventManagerComponent::AddHeisterToEventByCharacter_Implementation(FGameplayTag inEventTag, const UMETA_Character* inCharacter) {
    return false;
}
