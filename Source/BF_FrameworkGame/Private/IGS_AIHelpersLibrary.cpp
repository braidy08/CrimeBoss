...
AActor* UIGS_AIHelpersLibrary::GetPotentialTarget(const AIGS_GameCharacterFramework* inCharacter) {
    return NULL;
}
...
bool UIGS_AIHelpersLibrary::GetEnemyDetectorsForSideInRange(const UObject* inWCO, EIGS_TeamSideEnum inTeamSide, const FVector& InLocation, float InRange, TArray<AIGS_DetectorBase*>& outDetectors) {
    return false;
}

bool UIGS_AIHelpersLibrary::GetEnemyDetectorsForSide(const UObject* inWCO, EIGS_TeamSideEnum inTeamSide, TArray<AIGS_DetectorBase*>& outDetectors) {
    return false;
}
...
bool UIGS_AIHelpersLibrary::GetDetectorsOfSide(const UObject* inWCO, EIGS_TeamSideEnum inTeamSide, TArray<AIGS_DetectorBase*>& outDetectors) {
    return false;
}
...
float UIGS_AIHelpersLibrary::GetAngleToDirection2D(AIGS_GameCharacterFramework* inInstigator, FVector InDirection, const bool inFromEyes) {
    return 0.0f;
}
...
bool UIGS_AIHelpersLibrary::CanTargetBeRescued(const AIGS_GameCharacterFramework* inRescuer, const AIGS_GameCharacterFramework* inTarget) {
    return false;
}
