...
bool UMETA_UtilitiesLibrary::IsTagOfLevelledPerk(const FGameplayTag inPerkTag) {
    return false;
}
...
TArray<EMETA_ItemQuality> UMETA_UtilitiesLibrary::GetQualitiesFromBitmask(const int32 inBitmask) {
...
EMETA_ItemQuality UMETA_UtilitiesLibrary::GetHighestQualityFromBitmask(const int32 inBitmask) {
    return EMETA_ItemQuality::None;
}

FText UMETA_UtilitiesLibrary::GetFormattedBonusObjectiveDescription(const FMETA_ObjectiveConfig& InConfig) {
...
FGameplayTagContainer UMETA_UtilitiesLibrary::FilterByPerk(const FGameplayTagContainer& inPerkContainer, const FGameplayTag inPerkTag) {
