...
float UIGS_BadFlyGameplayStatics::GetDamageScaledByRadius(float DistanceFromEpicenter, float BaseDamage, float MinimumDamage, float DamageInnerRadius, float DamageOuterRadius, float DamageFalloff) {
    return 0.0f;
}
...
EIGS_BasePlatform UIGS_BadFlyGameplayStatics::GetBasePlatformType() {
    return EIGS_BasePlatform::None;
}
...
void UIGS_BadFlyGameplayStatics::ForceSetMissionWeights(UObject* inWCO, const int32 inTotalWeight) {
}
